...
4. **인터넷이 끊겨도 데모가 돈다.** 본선 중 외부 호출은 AI(활동 기록) 하나뿐이며 캐시·템플릿 폴백이 있다. **버스는 실시간도, 시간표도 쓰지 않는다** — 안내는 "노선번호 · 정거장 수 · 약 소요시간"뿐(우리 데이터가 보증하는 전부. 배차·시각표·실시간은 운영 이관 시 BIS 연동 로드맵). 데이터베이스 전체를 노트북에 복사하는 스크립트(`sync_local.sh`)로 시연장 와이파이가 죽어도 단독 완주한다.
...
- **포인트 적립은 상태 전이에 정확히 걸린다**: stamped +40, recorded +40, 완주(둘 다) +20 — 원장 테이블(point_ledger: session_id, quest_id, delta, reason) 1개에 기록, 100점 도달 시 칭호 자동 해금. 재추천은 무료(포인트와 무관). 가게 없는 퀘스트는 recorded만으로 완주 보너스 지급(합 60)
...
전제: 우리 데이터에는 "몇 시 몇 분에 버스가 온다"는 **시각표가 없다.** 있는 것은 정류장 위치·정류장별 경유 순서·좌표(CSV)뿐이고, **배차간격·첫차·막차도 없다**(태우 확정 7/31: 수기 보강도 하지 않음). 그래서 경로는 시각표 계산이 아니라 3단계로 만든다:

1. **어느 정류장에서 타나** — 사용자가 정류장을 직접 골랐으면 **그 정류장**(개인 위치 기준 정확 계산). 동만 골랐으면 동네 정류장 중 그 활동 장소 기준 접근성 점수(도보거리+노선 수)가 가장 좋은 정류장 — 이 경우 동 단위 근사이므로 화면에 "○○ 정류장 기준"을 표기한다. 점수표의 키가 (정류장×활동지)라 두 경로 모두 같은 표를 읽는다
...
3. **시간이 맞나 — 현재 시간이 개입하는 지점**: 사용자의 가능 시간 창에 [도보 + 대기·환승 버퍼(고정 10분) + 승차(거리 기반 추정) + 활동 시간]이 들어가는지 확인. 막차 검증은 하지 않는다(데이터 없음 — 데모는 낮 시간이라 무해, 운영 시 BIS 연동으로 보강). **배차간격·시각표·실시간 표시는 전부 하지 않는다**

카드에 적히는 것: "석사동 정류장에서 300번 · 7개 정거장 · 약 25분". **시각표·배차간격·실시간 어느 것도 안내하지 않는다** — 데이터가 보증하는 것(노선·정거장 수·추정 소요)만 표기한다. 시간대별 승하차 CSV는 경로 계산이 아니라 수요·상권 분석에 쓴다.
...
- 버스 안내는 "노선번호 · 정거장 수 · 약 소요시간"만 — 배차·시각표·실시간 등 데이터가 보증하지 않는 표시 금지
...
| 버스 접근성 (20%) | 접근성 점수표(6-1) 조회 — 도보거리·경유 노선 수의 종합. 요청 순간에 계산하지 않고 미리 계산해 둔 값을 읽는다 |
...
| 행정동 목록 | 경로 보유 동만 반환 (홈 출발지 선택) | R2 |
| 정류장 목록 | 동별 정류장 목록 조회 — 타이핑 필터는 화면에서 (출발지 직접 선택) | R2 |
| 퀘스트 시작 | started 전이 + 시작 시각 기록 | R2 |

포인트 잔액은 별도 API 없이 **세션·인증·기록 응답에 balance 필드로 포함**한다 (호출 1회 절약 + 적립 직후 최신 잔액 보장).
...
**대회 전 (P0 — 이슈로 등록돼 있음)**: 키 3종 발급(카카오맵·LLM·Supabase — TAGO 완전 불사용 확정), 데이터 10종 적재 완주 1회, 계약 문서 동결, 화면·조립 스파이크 검증, 스켈레톤 배포(화면↔서버↔DB 연결 확인 — 배선 문제를 본선 전에 소진).
