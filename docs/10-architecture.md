...
4. **인터넷이 끊겨도 데모가 돈다.** 본선 중 외부 호출은 AI(활동 기록) 하나뿐이며 캐시·템플릿 폴백이 있다. **버스는 실시간을 쓰지 않는다** — 배차간격 기반 "약 N분 간격" 안내로 통일(실시간 도착은 기획서상 '보완' 기능 — 운영 이관 시 시 BIS 연동 로드맵). 데이터베이스 전체를 노트북에 복사하는 스크립트(`sync_local.sh`)로 시연장 와이파이가 죽어도 단독 완주한다.
...
전제: 우리 데이터에는 "몇 시 몇 분에 버스가 온다"는 **시각표가 없다.** 있는 것은 정류장 위치·정류장별 경유 노선(CSV), 노선별 배차간격·첫차·막차(정적). 그래서 경로는 시각표 계산이 아니라 3단계로 만든다:

1. **어느 정류장에서 타나** — 사용자가 정류장을 직접 골랐으면 **그 정류장**(개인 위치 기준 정확 계산). 동만 골랐으면 동네 정류장 중 그 활동 장소 기준 접근성 점수(도보거리+노선 수+배차간격)가 가장 좋은 정류장 — 이 경우 동 단위 근사이므로 화면에 "○○ 정류장 기준"을 표기한다. 점수표의 키가 (정류장×활동지)라 두 경로 모두 같은 표를 읽는다
...
3. **시간이 맞나 — 현재 시간이 개입하는 지점**: 사용자의 가능 시간 창에 [도보 + 예상 대기 + 승차 + 활동 시간]이 들어가는지 확인(예상 대기 = 배차간격의 절반), 활동 종료가 그 노선 막차 이전인지 확인. **실시간 도착 표시는 하지 않는다** — 안내는 "약 N분 간격 운행"으로 통일 (기획서상 도착정보는 '보완' 기능. 운영 이관 시 시 BIS 연동 로드맵)

카드에 적히는 것: "석사동 정류장에서 300번, 약 12분 간격, 25분 소요". **"13시 42분 버스를 타세요" 같은 분 단위 시각표 안내는 하지 않는다** — 시각표 데이터가 없는데 흉내 내면 시연 중에 어긋난다. 시간대별 승하차 CSV는 경로 계산이 아니라 수요·상권 분석에 쓴다.
...
- 도착 안내 문구는 항상 "약 N분 간격 운행" — 데이터가 보증하지 않는 실시간 표시 금지
...
**대회 전 (P0 — 이슈로 등록돼 있음)**: 키 3종 발급(카카오맵·LLM·Supabase — TAGO 실시간은 미사용 확정, 노선·배차는 정적 수집), 데이터 10종 적재 완주 1회, 계약 문서 동결, 화면·조립 스파이크 검증, 스켈레톤 배포(화면↔서버↔DB 연결 확인 — 배선 문제를 본선 전에 소진).
