...
- **빈 결과 (8/1 결정)**: 완화 4단계 소진 후에도 0건이면 **정상 200**으로 `"cards": [], "more": []` + `relaxed: {"steps": [적용한 전 단계], "message": "지금 조건에 맞는 활동이 없어요"}` — recommendation_id·스냅샷은 그대로 발급(빈 결과율 = 대시보드 KPI 원천). 화면 동작은 25-screens 2장, 조립 규칙은 계약④ §4-2
