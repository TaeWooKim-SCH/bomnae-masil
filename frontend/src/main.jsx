...
function VerifyScreen() {
  ...
  // 조각은 서버가 완주(recorded) 시점에 채운다(계약 §5) — 획득 연출도 기록 저장 화면이 담당한다.
  // 스탬프 시점에 띄우면 홈 카드(서버 기준)와 숫자가 어긋난다.
...
function RecordScreen() {
  ...
  const [error, setError] = React.useState("");
  const [reveal, setReveal] = React.useState(null); // 조각 획득 연출 (#100 — 완주 시점)
  ...
  async function saveRecord() {
    ...
    try {
      ...
      const zoneCodeOfQuest = feature?.properties?.zone_code;
      if (feature && isCollectible(feature, collectionZoneMap) && !collectionZoneMap.collected.includes(zoneCodeOfQuest)) {
        revealNextRef.current = goArchive; // 연출을 보고 나서 보관함으로
        setReveal(feature);
      } else {
        goArchive();
      }
